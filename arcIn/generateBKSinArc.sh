bam_file=dan_hg38.bam
bam_file=$1
out_file=`echo $bam_file|perl -npe "s/.*\///"`
samtools view -h $bam_file |grep "^@SQ"|head -n 25|perl -npe "s/.*SN://;s/LN:/1\t/"|bedtools makewindows -b /dev/stdin -w 50000000 |perl -npe "s/\t/:/;s/\t/-/" |xargs -n 1 -P 120 -I{} bash generateBKSinArc_each.sh $bam_file {}
